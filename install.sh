...
apt-get update
apt-get --yes --force-yes install git bluez python3 python-gobject python3-cffi python3-dbus python3-alsaaudio python-configparser sound-theme-freedesktop vorbis-tools python3-pip icecast2 ezstream lame
echo "done."
...
if [ -d bt-speaker ]; then
  ...
  git clone https://github.com/MoralCode/sonos-bt-speaker.git
...
cp -n /opt/bt-speaker/icecast.xml /etc/icecast2/icecast.xml
